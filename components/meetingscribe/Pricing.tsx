            ...
            return (
              <Reveal key={plan.name} delay={i * 90}>
                ...
                >
                  ...
                  <div className="mb-6">
                    <p className={`text-sm font-semibold uppercase tracking-widest mb-1 ${highlight ? "text-blue-200" : "text-[#0055e0]"}`}>
                      {plan.name}
                    </p>
                    <div className="flex items-end gap-1 mb-3">
                      <span className={`text-5xl font-bold ${highlight ? "text-white" : "text-[#0a1628]"}`}>
                        €{plan.price}
                      </span>
                      <span className={`text-sm mb-2 ${highlight ? "text-blue-200" : "text-[#4a6080]"}`}>
                        {plan.period}
                      </span>
                    </div>
                    <p className={`text-sm leading-relaxed ${highlight ? "text-blue-100" : "text-[#4a6080]"}`}>
                      {plan.desc}
                    </p>
                  </div>
                  ...
                  <ul className="flex flex-col gap-2.5 mb-8 flex-1">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-2.5 text-sm">
                        <span className={`mt-0.5 shrink-0 font-bold ${highlight ? "text-[#00c8d8]" : "text-[#0055e0]"}`}>✓</span>
                        <span className={highlight ? "text-blue-50" : "text-[#4a6080]"}>{f}</span>
                      </li>
                    ))}
                    {plan.notIncluded.map((f) => (
                      <li key={f} className="flex items-start gap-2.5 text-sm opacity-40">
                        <span className="mt-0.5 shrink-0">✕</span>
                        <span className={highlight ? "text-blue-100" : "text-[#4a6080]"}>{f}</span>
                      </li>
                    ))}
                  </ul>
